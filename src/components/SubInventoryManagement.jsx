      ...
      {showPopup && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div className="bg-white p-6 rounded-lg shadow-lg max-w-lg w-full max-h-[90vh] overflow-y-auto">
            <h2 className="text-xl font-bold mb-4">{editingGroup ? 'Editar' : 'Nueva'} Referencia</h2>
            {errorMessage && <p className="text-red-500 mb-2">{errorMessage}</p>}
              ...
              <div className="grid grid-cols-4 gap-2">
                {sizes.map(size => (
                  <div key={size}>
                    ...
                    <input type="number" min="0" value={newReference.sizes[size] || 0} onChange={(e) => setNewReference({ ...newReference, sizes: { ...newReference.sizes, [size]: parseInt(e.target.value) || 0 } })} className="w-full p-1 border rounded" />
                  </div>
                ))}
              </div>
            </div>
